...
class cuda(Task.Task):
	run_str = '${NVCC} ${CUDAFLAGS} ${FRAMEWORKPATH_ST:FRAMEWORKPATH} ${CPPPATH_ST:INCPATHS} ${DEFINES_ST:DEFINES} ${CXX_SRC_F}${SRC} ${CXX_TGT_F}${TGT}'
	color   = 'GREEN'
	ext_in  = ['.h']
	vars    = ['CCDEPS']
	scan    = c_preproc.scan
...
def find_cuda_libs(self):
	...
	self.check_cxx(header='cublas.h', lib='cublas', libpath=_libpath, includes=_includes)
